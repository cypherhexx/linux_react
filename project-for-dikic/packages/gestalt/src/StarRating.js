...
class StarRatings extends React.Component {
  ...
  get renderStars() {
    ...
    const { highestStarHovered, highestStarActived } = this.state;

    const numberOfStarsArray = Array(...Array(numberOfStars));
    ...
    return numberOfStarsArray.map((_, index) => {
      ...
      const isActived = starRating <= highestStarActived;
      const isCurrentHoveredStar = starRating === highestStarHovered;
      const isCurrentActivedStar = starRating === highestStarActived;
      ...
      console.log(highestStarActived);
      return (
        <Star
          key={starRating}
          fillId={this.fillId}
          changeRating={
            changeRating
              ? () => {
                  changeRating(starRating, name);
                }
              : null
          }
          hoverOverStar={changeRating ? this.hoverOverStar(starRating) : null}
          mouseDown={this.activeStar(starRating)}
          mouseUp={this.unActiveStar()}
          isCurrentActivedStar={isCurrentActivedStar}
